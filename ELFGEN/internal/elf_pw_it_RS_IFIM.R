...
library(data.table);
library(scales);
library(jsonlite); #required for retrieving IFIM table
  ...
  if(nrow(data) && length(duplicates) > 3) {   
    ...
    if (nrow(upper.quant) > 3) {
      ...
      if (send_to_rest == 'YES') {
        ...
        #adminid <- paste(ifim_transect_name,search_code,"fe_quantreg_pwit_RS",x_metric,y_metric,quantile,station_agg,sampres,analysis_timespan,glo,ghi, sep='_');
      ...
      #Plot title
       ...
       #plot_title <- paste(Feature.Name," (",sampres," grouping)\n",startdate," to ",enddate,"\n\nQuantile Regression: (",round(glo,1)," < breakpoint < ",round(ghi,1)," = ",round(breakpt, digits = 1),") - PWIT ","\n\nIFIM Transect: ",ifim_transect_name,"\nDrainage Area = ",ifim_da_sqmi," mi^2",sep=""); 
      ...
      # START - plotting function
        ...
        geom_point(data = data_RS, show.legend = TRUE, colour = "slategray1")+
        geom_point(data = upper.quant_RS, show.legend = TRUE, colour = "palegreen")+
        #geom_quantile(data = data_RS, show.legend = TRUE,formula = y ~ x, quantiles = quantile, colour = "thistle4")+
        #geom_smooth(data = data_RS, show.legend = TRUE,method = "lm", colour = "pink", se = FALSE)+
        geom_smooth(data = upper.quant_RS, show.legend = TRUE,method = "lm", formula = y ~ x, colour = "tan1", se = FALSE)+
        ...
        #geom_smooth(data = data, method="lm",formula=y ~ x,show.legend = TRUE, aes(colour="tomato"),se=FALSE) + 
        ...
        scale_color_manual(
          "Legend",
          values=c("palegreen","slategray1","orange","black","red"),
          labels=c(EDAS_upper_legend,EDAS_lower_legend,Reg_upper_legend,"RCC bkpt")
        ) + 
        guides(
          colour = guide_legend(
            override.aes = list(
       ...
       #===============================================================
       #===============================================================                                                               linetype=c(0,0,1,1,3,2,4,5,6,5,5), 

...
for (x in 1:length(ifim_sites)){
  ...
  # model_flows <- om_get_rundata(207923, 11) 
  # model_flows$Qbaseline <- model_flows$Qout + (model_flows$wd_cumulative_mgd - model_flows$ps_cumulative_mgd ) * 1.547
  # f_0 <- model_flows[,c('thisdate', 'Qbaseline')]           
  # f_0$thisdate <- as.character(as.Date(index(model_flows))) 
  # names(f_0) <- c('Date', 'Flow')
  # flow.ts.range_0 <- f_0
  f_0 <- f_fxn(gage,0.0)

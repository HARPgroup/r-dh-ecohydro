...
#fim_sites <- c(397302) #Dunlap
#ifim_sites <- c(397303) #Craig
#ifim_sites <- c(397290) #Plains Mill 
ifim_sites <- c(397291) #Laurel Hill

stat_method <- "median" #"mean" or " median"
pctchg <- "10"

#===============================================================
#metrics <- c("riffle","fast_gen","pool_run","pool_cov","alg_mid")
metric_names <- c("Riffle Guild","Fast Generalist guild","Pool-run guild","Pool-cover guild","Algae and Midge Guild")
#metric_labels <- data.frame(metrics,metric_names)
...
#-------------------------------------------------------------------------------------------------
#label <- which(metric_labels$metrics == )

box_table_i <- data.frame(label = metric_names,
                        metric = targets,
                        flow = names(dfList[L]),
...
scale_y_continuous(limits = c(-12, 37))

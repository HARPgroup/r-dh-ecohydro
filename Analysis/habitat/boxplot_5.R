...
col_all = col_all[,!(names(col_all) %in% "month")]
#-------------------------------------------------------------------------------------------------
as.numeric(col_all[1,])
...
ggplot(box_table, aes(as.factor(flow),pctchg))+
  ...
  ggtitle(paste(ifim_site_name,":\n\nPercent Habitat Change with 10% Reduction in Flow Metric",sep=""))+
  xlab("Flow Metric (cfs)")+ 
...
#-------------------------------------------------------------------------------------------------
Jan_rows <- which(wua.at.q_0$month == "Jan")
Jan_data <- wua.at.q_0[Jan_rows,]

# mo_list <- c('maf', 'Jan')
# a <- 1
# for (a in 1:length(mo_list)) {
# table_i = data.frame(pctchg_all$pctchg)
# Jan_data <- c(paste("tbl_",mo_list[a],sep=""))

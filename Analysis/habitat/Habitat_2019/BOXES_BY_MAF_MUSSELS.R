...
#----------------------------------------------------------------------------------------------
#REMOVE NON-MUSSEL METRICS
all_sites <- mussels(all_sites)
...
reg_stats <- data.frame(month=character(),
  ...
  #SET PLOT FILE NAME
  plot_name <- paste((r-1),"-",month,"_boxplots_",stat,"_mussels",sep="") 
  ...
  #Add Habitat Breakpoint Label
    text(x = round(exp((-b)/m),1), y = 100, labels = "Habitat Breakpoint     ", pos=2, cex=1.8, font=3) 
    #Arrows(round(exp((-b)/m),1)+(0.1*round(exp((-b)/m),1)),100,round(exp((-b)/m),1),100,arr.type="simple",lwd=2,code=1)

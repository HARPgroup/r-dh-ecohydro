...
#----------------------------------------------------------------------------------------------
#Single metric tests
single_metric <- 'smb_adult' #'nh_adult','smb_adult
...
reg_stats <- data.frame(month=character(),
  ...
  #SET PLOT FILE NAME
  plot_name <- paste((r-1),"-",month,"_boxplots_",stat,"_",single_metric,sep="") 
  ...
  #Add Habitat Breakpoint Label
    text(x = round(exp((-b)/m),1), y = 25, labels = "    Habitat Breakpoint", pos=4, cex=1.8, font=3) 
    Arrows(round(exp((-b)/m),1)+(0.1*round(exp((-b)/m),1)),25,round(exp((-b)/m),1),25,arr.type="simple",lwd=2)

  #ADD LEGEND TO UPPER RIGHT CORNER
  legend(4150, 63, legend=c("Median Habitat Change", "Median Habitat Change Regression"),

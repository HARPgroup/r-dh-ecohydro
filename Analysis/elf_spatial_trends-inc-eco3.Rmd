...
# vars
yvar = 'aqbio_nt_total';#aqbio_nt_total, aqbio_nt_cent, aqbio_nt_darter, aqbio_nt_benins, aqbio_nt_minnow, aqbio_nt_cypr_native
xvar = 'erom_q0001e_mean';
graphtitle = "Total # of Taxa, Fish"
quant = 0.8
pmin <- 0.01
pmin10 <- 0.01
Rmin <- 0.0
...
huc10quant = 0.8; #use 0.8 for bpj-530, 0.5 for bpj-q50
huc10_tag = 'bpj-530'
huc10_rawdata = 
  fn_dh_elfstats ( ftype = 'fe_elf',
    site = base_url, analysis_timespan = "full", yvar = yvar,
    sampres = "species", stat_quantreg_qu = huc10quant, feature_ftype = 'nhd_huc10', stat_quantreg_ghi = 'all',
    stat_quantreg_glo = 'all', dataset_tag = huc10_tag
  );
...
huc10_xvar_data = subset(huc10_xvar_data, out_p <= pmin10)
...
sseco <- elf_quantile_table(data = ecoiii_xvar_data)
...
fit <- aov(out_rsq_adj ~ resolution, data=alldata)
ssaov = summary(fit)
fitm <- aov(out_m ~ resolution, data=alldata)
maov = summary(fitm)
data6and10 = subset(alldata, resolution != "huc8")
fitm6ten <- aov(out_m ~ resolution, data=data6and10)
maov6ten = summary(fitm6ten)
...
pander(ss6, table.split.table=Inf, caption='huc 6')
pander(ss8, table.split.table=Inf, caption='huc 8')
pander(ss10, table.split.table=Inf, caption='huc 10')
pander(quantile(huc6_xvar_data$out_m), table.split.table=Inf, caption='huc 6 m')
pander(quantile(huc8_xvar_data$out_m), table.split.table=Inf, caption='huc 8 m')
pander(quantile(huc10_xvar_data$out_m), table.split.table=Inf, caption='huc 10 m')
pander(sseco, table.split.table=Inf, caption='Ecoregion iii')
pander(ssaov, table.split.table=Inf, caption='Comparison of R2 for Different Watershed Sizes')
pander(maov, table.split.table=Inf, caption='Comparison of m for Different Watershed Sizes')
